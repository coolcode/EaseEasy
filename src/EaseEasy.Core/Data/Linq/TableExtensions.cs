...
namespace CoolCode.Data.Linq
{
	public static class TableExtensions
	{
		...
		{
			DbCommand delete = table.GetDeleteCommand<TEntity>(entities);
		...
		/// <remarks>This method is useful for debugging purposes or when used in other utilities such as LINQPad.</remarks>
		public static string DeletePreview<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities) where TEntity : class
		{
			DbCommand delete = table.GetDeleteCommand<TEntity>(entities);
			return delete.PreviewCommandText(false) + table.Context.GetLog();
		}
		...
		/// <returns>Returns a string representation the LINQ <see cref="IProvider"/> command text and parameters used that would be issued to delete all entities from the collection with a single delete command.</returns>
		/// <remarks>This method is useful for debugging purposes or when used in other utilities such as LINQPad.</remarks>
		public static string DeletePreview<TEntity>(this Table<TEntity> table, Expression<Func<TEntity, bool>> filter) where TEntity : class
		{
			return table.DeletePreview(table.Where(filter));
		}
		...
		public static string DeleteSQL<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities) where TEntity : class
		{
			DbCommand delete = table.GetDeleteCommand<TEntity>(entities);
		...
		/// <returns>Returns the Transact SQL string representation the LINQ <see cref="IProvider"/> command text and parameters used that would be issued to delete all entities from the collection with a single delete command.</returns>
		/// <remarks>This method is useful for debugging purposes or when LINQ generated queries need to be passed to developers without LINQ/LINQPad.</remarks>
		public static string DeleteSQL<TEntity>(this Table<TEntity> table, Expression<Func<TEntity, bool>> filter) where TEntity : class
		{
			return table.DeleteSQL(table.Where(filter));
		}
		...
		/// <remarks>This method is useful for debugging purposes or when used in other utilities such as LINQPad.</remarks>
		public static string UpdatePreview<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities, Expression<Func<TEntity, TEntity>> evaluator) where TEntity : class
		{
			DbCommand update = table.GetUpdateCommand<TEntity>(entities, evaluator);
		...
		/// <returns>Returns a string representation the LINQ <see cref="IProvider"/> command text and parameters used that would be issued to update all entities from the collection with a single update command.</returns>
		/// <remarks>This method is useful for debugging purposes or when used in other utilities such as LINQPad.</remarks>
		public static string UpdatePreview<TEntity>(this Table<TEntity> table, Expression<Func<TEntity, bool>> filter, Expression<Func<TEntity, TEntity>> evaluator) where TEntity : class
		{
			return table.UpdatePreview(table.Where(filter), evaluator);
		}
		...
		/// <remarks>This method is useful for debugging purposes or when LINQ generated queries need to be passed to developers without LINQ/LINQPad.</remarks>
		public static string UpdateSQL<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities, Expression<Func<TEntity, TEntity>> evaluator) where TEntity : class
		{
			DbCommand update = table.GetUpdateCommand<TEntity>(entities, evaluator);
		...
		/// <returns>Returns the Transact SQL string representation the LINQ <see cref="IProvider"/> command text and parameters used that would be issued to update all entities from the collection with a single update command.</returns>
		/// <remarks>This method is useful for debugging purposes or when LINQ generated queries need to be passed to developers without LINQ/LINQPad.</remarks>
		public static string UpdateSQL<TEntity>(this Table<TEntity> table, Expression<Func<TEntity, bool>> filter, Expression<Func<TEntity, TEntity>> evaluator) where TEntity : class
		{
			return table.UpdateSQL(table.Where(filter), evaluator);
		}
		...
		private static DbCommand GetDeleteCommand<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities) where TEntity : class
		{
			var deleteCommand = table.Context.GetCommand(entities);
			deleteCommand.CommandText = string.Format("DELETE {0}\r\n", table.GetDbName()) + GetJoinQuery<TEntity>(table, entities);
			return deleteCommand;
		}

		private static DbCommand GetUpdateCommand<TEntity>(this Table<TEntity> table, IQueryable<TEntity> entities, Expression<Func<TEntity, TEntity>> evaluator) where TEntity : class
		{
			...
			updateCommand.CommandText = string.Format("UPDATE {0}\r\n{1}\r\n\r\n{2}",
															...
															GetJoinQuery<TEntity>(table, entities));	// Subquery join created from entities command text
		...
		///		select h;
		///		
		/// HistoryData.UpdatePreview( pay, h => new HistoryData { hisIndex = ( int.Parse( h.hisIndex ) - 1 ).ToString() } ).Dump();
		/// 
		...
		private static string GetJoinQuery<TEntity>(Table<TEntity> table, IQueryable<TEntity> entities) where TEntity : class
		{
			var metaTable = table.Context.Mapping.GetTable(typeof(TEntity));
		...
		public static int Delete2<T>(this Table<T> table, Expression<Func<T, bool>> predicate) where T : class
		{
			//获取表名
		...
		public static int Update2<T>(this Table<T> table, Expression<Func<T, bool>> predicate, Expression<Func<T, T>> updater) where T : class
		{
			...
			Func<Expression, object> getParameterValue = expr =>
			{
				object value  = Expression.Lambda(expr , null).Compile().DynamicInvoke();
				return value;
			};
			...
			var updateMemberCollection = updateMemberExpr.Bindings.Cast<MemberAssignment>().Select(c =>
			{
				...
				p.Value = getParameterValue(c.Expression);
				return p;
			})
			.ToArray();
