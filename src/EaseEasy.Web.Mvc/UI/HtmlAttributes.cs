	...
	public class HtmlAttributes : IHtmlAttributes {
		...
		*/
		...
		void ICollection.CopyTo(Array array, int index) {
			((ICollection)_attributes).CopyTo(array, index);
		}

		bool ICollection.IsSynchronized {
			get { return ((ICollection)_attributes).IsSynchronized; }
		}

		object ICollection.SyncRoot {
			get { return ((ICollection)_attributes).SyncRoot; }
		}
		...
		void ICollection<KeyValuePair<string, string>>.CopyTo(KeyValuePair<string, string>[] array, int arrayIndex) {
			((IDictionary)_attributes).CopyTo(array, arrayIndex);
		}
		...
		public override string ToString() {
			...
			return string.Join(" ", this.Select(pair => string.Format(attributeFormat, pair.Key, pair.Value)).ToArray());
		}
	}
}
